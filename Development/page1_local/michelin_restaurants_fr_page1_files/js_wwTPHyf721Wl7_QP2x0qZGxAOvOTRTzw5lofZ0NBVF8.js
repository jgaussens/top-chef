...
/**
 * @file
 * JS file; contains code to initiate slideout menu.
 */

Drupal.behaviors.mrSlidingMenu = {
  attach: function (context, settings) {
    jQuery('body').once('sliding-menu', function() {
      var ua = navigator.userAgent.toLowerCase();
      if (ua.indexOf('mobile') >= 0 || ua.indexOf('tablet') >= 0 || ua.indexOf('ipad') >= 0) {
        jQuery('.l-header').addClass('fixed');
        
        settings.slideout = new Slideout({
          'panel': jQuery('.l-content-wrapper')[0],
          'menu': document.getElementById('mr-b2c-sliding-menu'),
          'duration': 200,
          'fx': 'ease',
          'padding': 256,
          'tolerance': 70,
        ...
        jQuery('.js-slideout-toggle').bind('click', function() {
          Drupal.settings.slideout.toggle();
        });

        jQuery('.close-sliding-menu').bind('click', function(eve) {
          Drupal.settings.slideout.close();
        });
        
        settings.slideout.on('beforeopen', function() {
          jQuery('.fixed').addClass('fixed-open');
        });

        settings.slideout.on('beforeclose', function() {
          jQuery('.fixed').removeClass('fixed-open');
        });
      }
      else {
        jQuery('#mr-b2c-sliding-menu').addClass('mr-dropdown-menu');
        jQuery('.header-menu-trigger-wrapper').append(jQuery('#mr-b2c-sliding-menu'));
        jQuery('.header-menu-trigger-wrapper').on('click',function() {
          jQuery('.mr-dropdown-menu').toggle();
          jQuery('.header-menu-trigger-wrapper .hamburger span').toggleClass('is-open');
        });
      }
    });
  }
...
(function ($) {
  Drupal.behaviors.seoGoogleTagManager = {
    attach: function (context, settings) {
      $('[attr-gtm-jsevent]').each(function () {
        $(this).once('gtm-jsevent', function() {
          var jsevent = $(this).attr('attr-gtm-jsevent');
          $(this).bind(jsevent, function () {
            try {
              var title = $(this).attr('attr-gtm-title');
              if (title == '' && $(this).prop("tagName").toLowerCase() == 'select') {
                title = $(this).find('option:selected').text();
              }
              var data = {
                'event': $(this).attr('attr-gtm-event'),
                'type': $(this).attr('attr-gtm-type'),
                'title': title,
              };
              // @TODO: Need to confirm if this works perfectly fine for <a>.
              // It will be same even if we do in onclick though.
              dataLayer.push(data);
            }
            catch (e) {
            }
      ...
      // Create a generic event on change on Select2 select list.
      $('select.use-select-2').on('change', function(e) {
        try {
          var data = {
